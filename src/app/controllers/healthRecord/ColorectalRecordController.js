...
class ColorectalRecordController {
  ...
  getById = async (req, res) => {
    try {
      ...
      const canAccess = await authenticateToken(authHeader);
      if (canAccess == 1) {
        const record = await ColorectalRecordModel.findById(req.body.id);
  ...
  getByIdentID = async (req, res) => {
    const jsonData = req.body; // dữ liệu từ web frontend gửi lên

    const content = fs.readFileSync(
      path.resolve(__dirname, "templates/templateCase.docx"),
      "binary"
    );
    ...
    res.download(outputPath, "Ket_Qua_Giai_Trinh_Tu_Gen.docx");
    // try {
    //     const record = await ColorectalRecordModel.findOne({
    //         identID: req.body.identID,
    //     });
    //     if (record) {
    //         return res.status(200).json({
    //             success: true,
